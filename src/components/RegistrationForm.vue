<template>
  <div class="form">
    ...
    <button class="form__button" @click="_click" v-if="!loading">Зарегистрироваться</button>
    <button class="form__button form__button--disable" @click.prevent v-else>Регистрация...</button>
  </div>
</template>
    ...
    methods: {
      _click () {
        if (this.userName.length > 0 && this.password.length > 0) {
          this.$store.dispatch('registration', { userName: this.userName, password: this.password })
        }
